...
EOSQL
    my $msgs = [ map { _prepare_msg($_, $session) } @{ $dbh->selectall_arrayref( $sql, undef, $session->{tex_id}, $session->{user} ) } ];
...
EOSQL
    my $notes = [ map { _prepare_note($_) } @{ $dbh->selectall_arrayref( $sql ) } ];
